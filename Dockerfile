# BioCLIP 2.5 Species Classifier — Sage/Waggle edge plugin
# Default model: BioCLIP 2.5 Huge (imageomics/bioclip-2.5-vith14, ViT-H/14)
# Text embeddings: TreeOfLife-200M (219M+ biological images, 450K+ species)
...
# Patch pybioclip to support BioCLIP 2.5 Huge.
# pybioclip 2.1.5 only knows about bioclip and bioclip-2 in its TOL_MODELS.
# This adds BioCLIP 2.5 and points at its model-specific embedding files
# in the TreeOfLife-200M dataset repo.
COPY patch_pybioclip.py /tmp/patch_pybioclip.py
RUN python3 /tmp/patch_pybioclip.py && rm /tmp/patch_pybioclip.py

# Pre-download BioCLIP 2.5 Huge model weights and TreeOfLife-200M embeddings
...
# invalidate the expensive (~4-5 GB) model + embeddings download layer.
RUN python3 -c "\
from bioclip.predict import TreeOfLifeClassifier; \
c = TreeOfLifeClassifier(model_str='hf-hub:imageomics/bioclip-2.5-vith14'); \
from bioclip import Rank; \
from PIL import Image; \
import numpy as np; \
dummy = Image.fromarray(np.zeros((224,224,3), dtype=np.uint8)); \
c.predict([dummy], rank=Rank.CLASS, k=1); \
print('BioCLIP 2.5 Huge model + embeddings cached')"
